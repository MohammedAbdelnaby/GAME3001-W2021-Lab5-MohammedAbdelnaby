...
bool CollisionManager::LOSCheck(glm::vec2 start_point, glm::vec2 end_point, const std::vector<DisplayObject*> object, DisplayObject* target)
{
	for (auto obj : object)
	{
		auto objectOffset = glm::vec2(obj->getWidth() * 0.5f, obj->getHeight() * 0.5f);
		if (lineRectCheck(start_point, end_point, obj->getTransform()->position - objectOffset, obj->getWidth(), obj->getHeight()))
		{
